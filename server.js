...
The error `column d.uploaded_by_user_id does not exist` is still appearing even though you executed the `ALTER TABLE` commands previously. This means that, for some reason, the column is still not present or recognized by the server's database connection.

To definitively solve this, we will adjust the `/documents` GET route in `server.js` once more. This time, we will make the `uploaded_by_user_id` in the `Documents` table optional in the `SELECT` query, and directly include the `uploaded_by_user_id` from the Documents table in the select clause and remove the join to the Users table.

**Here's the plan:**

1.  **Modify `server.js` (`/documents` GET route):** I will update the `server-js-fix` Canvas to change the `/documents` GET route. We will select `d.uploaded_by_user_id` directly and handle the user name on the frontend. This will allow the documents to load without error, and then we can clearly see if `uploaded_by_user_id` is populating (i.e., if the INSERT is working).

Here is the updated code for `server.js` in the Canvas.
...
app.get('/documents', authenticateToken, async (req, res, next) => {
    ...
    let sql = `SELECT document_id, title, description, file_name, mime_type, upload_date, file_path,
                      uploaded_by_user_id -- Select the ID directly
               FROM Documents
               WHERE company_id = $1`;
