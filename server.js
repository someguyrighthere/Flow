...
app.post('/shifts/auto-generate', isAuthenticated, isAdmin, async (req, res) => {
    ...
    try {
        ...
        let employeeScheduleData = employees.map(e => ({
            ...
            // IMPORTANT: 'scheduledForCurrentDay' will be added/managed within the daily loop for cleaner reset
        }));
        ...
        for (let i = 0; i < 7; i++) {
            ...
            let currentDayEmployees = employeeScheduleData.map(emp => ({ ...emp, scheduledForCurrentDay: false }));
            ...
            for (let currentHour = businessStartHour; currentHour < businessEndHour; currentHour++) {
                ...
                if (eligibleFTEmployees.length > 0) {
                    ...
                    employeeScheduled.scheduledForCurrentDay = true; // Mark as scheduled for THIS day's current iteration
                    ...
                    for (let h = currentHour; h < currentHour + FULL_TIME_WORK_DURATION; h++) { 
                        ...
                        if (idx >= 0 && idx >= 0 && idx < dailyCoverage.length) dailyCoverage[idx] = true; // Fix: Ensure idx is not negative
                    }
                ...
                if (eligiblePTEmployees.length > 0) {
                    ...
                    employeeScheduled.scheduledForCurrentDay = true; // Mark as scheduled for today
                ...
                // If this hour could not be scheduled by FT or PT, the loop will naturally advance to the next currentHour.
            }
        }
...
startServer();
I understand you're reporting that the auto-scheduler is still not working as intended, and you've provided new screenshots of your client-side console (`image_ff75d7.png`) and server logs (`image_ff762f.png`).

**The critical error is coming from your server-side `server.js`:**

* **Server Log (`image_ff762f.png`):** `Auto-scheduling failed: ReferenceError: employeeAssignedThisHour is not defined at /opt/render/project/src/server.js:450:21`
* **Client-side Console (`image_ff75d7.png`):** Shows the 500 Internal Server Error, which is a result of the server crashing due to the `ReferenceError`.

**What this means:**

The `ReferenceError` on `server.js:450` means that the variable `employeeAssignedThisHour` is being used before it has been declared in the scope where it's referenced.

Looking at your `server.js` (from the Canvas), `employeeAssignedThisHour` is only declared and assigned a value *inside* the `if (eligibleFTEmployees.length > 0)` block (for full-time employees). If that block is not executed (i.e., no full-time employees are scheduled), then `employeeAssignedThisHour` never gets defined. When the code then reaches the part-time scheduling logic and attempts to check `if (!employeeAssignedThisHour && remainingDailyTargetHours > 0)`, it tries to access an undeclared variable, causing the `ReferenceError` and crashing your server.

**To fix this `ReferenceError` and get the auto-scheduler running, we need to:**

1.  **Declare `employeeAssignedThisHour` at a higher scope:** Initialize it to `null` (or `false`) at the beginning of the daily loop, before any full-time or part-time scheduling attempts.
2.  **Consistently update `employeeAssignedThisHour`:** When either a full-time or part-time employee is successfully scheduled, make sure `employeeAssignedThisHour` is updated with the scheduled employee object.

This will ensure the variable always exists when checked later in the code.

**Here's the updated `server.js` file in the Canvas. Please replace the entire content of your local `server.js` file with this code.**

After updating, **remember to restart your Node.js server** for the changes to take effect. Then, try running the auto-scheduler again from your scheduling page and observe the calendar. This should resolve the server crash and allow us to see the effects of the auto-scheduling logic.
...
startServer();I understand you're reporting that the auto-scheduler is still not working as intended. You've provided two new screenshots: one of your client-side console (`image_ff75d7.png`) and one of your server logs (`image_ff762f.png`).

**The critical error is coming from your server-side `server.js`:**

* **Server Log (`image_ff762f.png`):** `Auto-scheduling failed: ReferenceError: employeeAssignedThisHour is not defined at /opt/render/project/src/server.js:450:21`
* **Client-side Console (`image_ff75d7.png`):** Shows the 500 Internal Server Error, which is a result of the server crashing.

**What this means:**

The `ReferenceError` on `server.js:450` means that the variable `employeeAssignedThisHour` is being used before it has been declared in the scope where it's referenced.

Looking at your `server.js` (from the Canvas), `employeeAssignedThisHour` is only declared and assigned a value *inside* the `if (eligibleFTEmployees.length > 0)` block (for full-time employees). If that block is not executed (i.e., no full-time employee is scheduled), then `employeeAssignedThisHour` is never created. When the code then reaches the part-time scheduling logic and attempts to check `if (!employeeAssignedThisHour && remainingDailyTargetHours > 0)`, it tries to access an undeclared variable, causing the `ReferenceError` and crashing your server.

**To fix this `ReferenceError`, we need to:**

1.  **Declare `employeeAssignedThisHour` at a higher scope:** Initialize it to `null` at the beginning of the daily loop for each day.
2.  **Consistently update `employeeAssignedThisHour`:** When either a full-time or part-time employee is successfully scheduled, make sure `employeeAssignedThisHour` is updated with the scheduled employee object.

**Here's the updated `server.js` file in the Canvas:**
    ...
    try {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            for (let currentHour = businessStartHour; currentHour < businessEndHour; currentHour++) {
                ...
                // NEW: Declare employeeAssignedThisHour at a higher scope within the loop
                ...
                if (eligibleFTEmployees.length > 0) {
                    ...
                    // No 'continue' here, just let the loop iterate to the next 'currentHour'
                }
                ...
                // The outer 'for' loop handles currentHour increment.
            }
        }
...
startServer();
I understand you're reporting the auto-scheduler is still not working as intended, and you've provided the server logs (`image_ff762f.png` shows a `ReferenceError: employeeAssignedThisHour is not defined`) and client-side console output (`image_ff75d7.png` shows the 500 error).

My apologies for the persistent issues with this complex auto-scheduling logic. We need to fix both the `ReferenceError` on the server and refine the scheduling algorithm's logic.

**Problem Summary:**

1.  **`ReferenceError: employeeAssignedThisHour is not defined`:** This is a direct result of `employeeAssignedThisHour` being declared within an `if` block, making it inaccessible outside that block when referenced.
2.  **Multiple Shifts Per Employee Per Day:** The `scheduledForCurrentDay` flag, while added, isn't fully preventing multiple shifts on the same day for a single employee in its current implementation. The loop continues for `currentHour` iterations, potentially finding new slots for an already-scheduled employee.
3.  **Scheduling Outside Business Hours:** Shifts are still appearing outside the `businessStartHour` and `businessEndHour`.

**Here's the plan to fix these critical issues in your `server.js` file (within the "Updated server.js (Auto-Schedule Fixes)" Canvas):**

1.  **Fix `ReferenceError`:** Declare `employeeAssignedThisHour` at the start of the daily loop (outside any `if` conditions) to ensure it's always in scope.
2.  **Robust "One Shift Per Employee Per Day":** Instead of relying solely on `scheduledForCurrentDay` within the filter, we will ensure that once an employee is assigned a shift, they are effectively "removed" from the pool of *currently eligible employees for the remainder of that specific day's scheduling*. This will involve updating the `currentDayEmployees` array directly.
3.  **Strict Time Boundary Enforcement:**
    * Ensure `currentHour` for new shifts starts strictly from `businessStartHour`.
    * Verify that the calculated `shiftEndTime` does not exceed `businessEndHour`.
    * Use more explicit `Date` object construction on the server to handle timezones robustly when creating `start_time` and `end_time` for PostgreSQL's `TIMESTAMPTZ` columns, reducing potential timezone-related shifts.

**I am providing the completely revised `server.js` file below with these comprehensive fixes. Please replace the entire content of your local `server.js` file with this code.**

After updating, **remember to restart your Node.js server** for the changes to take effect. Then, try running the auto-scheduler again from your scheduling page and observe the calendar. This should resolve the issues of shifts outside business hours and multiple shifts per employee per day.
...
app.post('/shifts/auto-generate', isAuthenticated, isAdmin, async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            // Create a temporary daily state for employees to track who has been scheduled for *this specific day*
            // Initialize here to be in scope for both FT and PT sections
            let currentDayEmployees = employeeScheduleData.map(emp => ({ ...emp, scheduledForCurrentDay: false })); 
            let employeeScheduledThisHour = null; // NEW: Track if ANY employee was assigned a shift in this currentHour iteration
            ...
            for (let currentHour = businessStartHour; currentHour < businessEndHour; currentHour++) {
                ...
                // Step 2: If no FT was scheduled for this slot, try to schedule a Part-time employee
                // Only attempt if no employee was assigned in the FT step for this currentHour, and still target hours left
                if (!employeeAssignedThisHour && remainingDailyTargetHours > 0) { // Check employeeAssignedThisHour here
                    const eligiblePTEmployees = currentDayEmployees.filter(emp => { // Filter from currentDayEmployees
                        if (emp.employment_type !== 'Part-time') return false; 
                        if (emp.daysWorked >= 5) return false; 
                        if (emp.scheduledForCurrentDay) return false; // Skip if already scheduled today
