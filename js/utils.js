    ...
    if (inviteEmployeeForm) {
        inviteEmployeeForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // NEW: Check for specific employee limit error message
                const employeeLimitMessage = "Free tier is limited to 5 employees. Please upgrade your plan to invite more.";
                const proLimitMessage = "Pro plan is limited to 100 employees. Please upgrade to the Enterprise plan to invite more.";

                if (error.message && (error.message.includes(employeeLimitMessage) || error.message.includes(proLimitMessage))) {
                    // Custom modal for upgrade prompt
                    const upgradePrompt = `
                        <p>${error.message}</p>
                        <p style="margin-top: 15px;">
                            <a href="pricing.html" class="btn btn-primary" style="text-decoration: none; display: inline-block; padding: 10px 20px; border-radius: 8px; margin-top: 10px;">Upgrade Plan</a>
                        </p>
                    `;
                    showModalMessage(upgradePrompt, true); // Display as error, but with link
                } else {
